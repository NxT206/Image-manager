...
export function openTagSelectorModal(filename, imagesMap, onUpdate) {
  ...
  box.style.cssText = `background: var(--panel-bg, #1e1e24); border: 1px solid var(--border-color, #2e2e38); padding: 20px; border-radius: 8px; width: 300px; max-width: 90vw; display: flex; flex-direction: column; gap: 12px; color: var(--text-main, #f4f4f6);`;
  ...
  listContainer.style.cssText = `max-height:150px; overflow-y:auto; display:flex; flex-direction:column; gap:4px;`;
  ...
  function renderList(filter = "") {
    ...
    filtered.forEach(tag => {
      ...
      item.onclick = () => {
        if (isAttached) currentData.tags = currentData.tags.filter(t => t !== tag);
        else currentData.tags.push(tag);
        onUpdate();
  ...
  box.append(title, searchInput, listContainer, closeBtn);
